  ...
  //  let timerPicker = UIDatePicker()
    override func viewDidLoad() {
       ...
       // timer = Timer.scheduledTimer(timeInterval: 0.1, target: self, selector: #selector(UpdateTime), userInfo: nil, repeats: true) // Repeat "func Update() " every second and update the l
        timerLabel.font = UIFont.preferredFont(forTextStyle: .body)
        timerLabel.adjustsFontForContentSizeCategory = true
    
        timerPicker.datePickerMode = .countDownTimer
        timerPicker.minuteInterval = 1 // Установите интервал в минутах
        let hours = 1
        let minutes = 30
        let seconds = 45
        let totalSeconds = hours * 3600 + minutes * 60 + seconds
        timerPicker.countDownDuration = TimeInterval(totalSeconds) // Установите общее количество секунд
